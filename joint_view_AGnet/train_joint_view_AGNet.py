##########################-The code is modified to train on joint view action detetcion with a 2 stream network -##################
...
parser.add_argument('-num_classes', type=str, default='False')
...
def run_network(model, data, gpu, epoch=0, baseline=False):
    ...
    activation = model(inputs_1,inputs_2,mask_new,mask_new)
...
def train_step(model, gpu, optimizer, dataloader, epoch):
    ...
    for data in dataloader:
        ...
        #print("hello", probs.data.cpu().numpy()[0].shape, data[2].numpy()[0].shape)
...
if __name__ == '__main__':
    ...
    if args.train:
        ...
        if args.model=="TCN":
            ...
            from AGNet_modified import AGNet as Net  #chnage here
            rgb_model = Net(mid_channel, 512,input_channnel, classes)
